print("<DESCRIPTION>Test description :-  This test run for Frequency modulation of a given carrier signal, with different baseband frequency and given deviation</DESCRIPTION>")
###########################
###START OF SCRIPT###
...
for fm_freq in fm_frequencies:
    ...
    if config.spectrum:  # spectrum commands for automation
        ...
        if (error_value < (frequency_th * freq_in)):# and (power_max > threshold):  # Condition to conclude the test result
            ...
            devicePrintResp.Print()
            fm_error = fm_freq_response - (fm_freq/1e6)
