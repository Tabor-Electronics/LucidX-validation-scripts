print("<DESCRIPTION>Test description :-  This test run for Amplitude modulation of a given carrier signal, with external  baseband frequency and deviation</DESCRIPTION>")
###########################
###START OF SCRIPT###
...
if config.spectrum:
    ...
    if status:
        spectrum_methods.set_reference_power(config.power_default + 15,spectrum_analyzer)  # step 1) set reference power level on spectrum
...
for ext_freq in am_ext_frequencies:
    ...
    # print(
    #     "Press on the Peak search button on Spectrum analyzer and a delta marker on the left/right on the peak and Note down modulation frequency and and check for AM signal")
            ...
            if fm_freq_response != 0:
                ...
                if (fm_error < (0.2 * ext_freq / 1e6)) and power_max_r > threshold:
                    devicePrintCmd.msg_user.set(
                        'Test pass for Frequency = {0} Hz at Carrier frequency of {1} MHz'.format(ext_freq, freq_in))
                    devicePrintCmd.Print()
                else:
                    devicePrintCmd.msg_user.set(
                        'Test Fail for FM Frequency = {0} Hz at Carrier frequency of {1} MHz'.format(ext_freq, freq_in))
                    devicePrintCmd.Print()
            else:
