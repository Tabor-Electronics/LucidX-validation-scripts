...
'''
frequency_list = [1000e6, 2000e6, 3000e6]  # ,4e9,5e9] #frequency in Hz
...
if config.spectrum:
	...
	for i in range(1):#range(len(frequency_list)-1):
		desired_freq = frequency_list[i + 1] / 1e6
		desired_power = power_list[i + 1]
	...
	# input()
		...
		if freq_error < fr_th and power_error < pr_th:
			...
			devicePrintCmd.Print()
	devicePrintCmd.msg_user.set('Press enter to finish this test for bus trigger with step')
	devicePrintCmd.Print()
	input()
