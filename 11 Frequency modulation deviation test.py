print("<DESCRIPTION>Test description :-  This test run for Frequency modulation of a given carrier signal, given baseband frequency and with different deviation</DESCRIPTION>")
###########################
###START OF SCRIPT###
...
# Global Parameters
freq_in = 2000#config.frequency_default
...
for deviation in deviations:
    fm_freq=deviation/100
    # Internal source commands for FM
    fm_source_q, fm_freq_q, fm_dev_q, status = FrequencyModulation.frequency_modulation_internal_on(fm_freq, deviation)
    ...
    if config.spectrum:  # spectrum commands for automation
        spectrum_analyzer, status = spectrum_methods.reset(config)
        # spectrum_methods.set_trace_mode_clear(spectrum_analyzer)
        ...
        if (error_value < (frequency_th * freq_in)):  # and (power_max > threshold):  # Condition to conclude the test result
            span = 8 * deviation/1e6
            BW = 0.75*fm_freq/1e6
            spectrum_methods.set_span_freq(span, spectrum_analyzer)
            spectrum_methods.set_resolution_bandwidth(BW, spectrum_analyzer)
            spectrum_methods.set_trace_mode_max(spectrum_analyzer)
            freq_out_dev, power_max_dev = spectrum_methods.set_marker(spectrum_analyzer)  # Read marker x (frequency) and y (power)
            deve = abs(freq_out_dev-freq_in)
            error= abs(deve-deviation/1e6)
            if (error<0.2*deviation/1e6):
                devicePrintResp.msg_gui.set('fm  freq={0}::p0.00::n0.00,Deviation={1}::p0.00::n0.00'.format(fm_freq, deve))
                devicePrintResp.Print()
                devicePrintCmd.msg_user.set('Test pass for FM Frequency = {0} Hz and deviationn {1} Hz at Carrier frequency of {2} MHz'.format(fm_freq,deviation, freq_in))
                devicePrintCmd.Print()
            else:
                devicePrintCmd.msg_user.set('Test Fail for FM Frequency = {0} Hz and deviationn {1} Hz at Carrier frequency of {2} MHz'.format(fm_freq,deviation, freq_in))
                devicePrintCmd.Print()
        else:
